...
// sort array according to its priority.
void PQArray::sortArray() {
...
//just for test the time complexity
void PQArray::add(DataPoint elem) {
    if(size()==_numAllocated)
        extendArray();
    _elements[size()] = elem;
    _numFilled++;
...
/*
 * Big-O Test:
 * Student test is at the end of this file.
 * Here I record the data size and time used for fillQueue and emptyQueue.
 * ------test for fillQueue:------
 * data size:       time(s):
 * 10000            0.830
 * 20000            3.257
...
STUDENT_TEST("Big-O test, fillQueue") {
    PQArray pq;
    int n = 10000;
    for (int i=0;i<4;i++)
    {
...
void fillAdd(PQArray& pq, int n) {
    pq.clear(); // start with empty queue
    for (int i = 0; i < n; i++) {
        pq.add({ "", randomReal(0, 10) });
    }
}

STUDENT_TEST("Big-O test, emptyQueue") {
    PQArray pq;
    int n = 8e7;
    for (int i=0;i<4;i++)
    {

        TIME_OPERATION(n, fillAdd(pq, n));
        TIME_OPERATION(n, emptyQueue(pq, n));
        n*=2;
    }
}
